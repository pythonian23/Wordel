...
class Wordel:
    def __init__(self, *args, **kwargs):
        source, self.length, self.allowed, self.hard, self.debug \
            = utils.kwarg_parser({
    ...
    def run(self, ans=None):
        ...
        def format_keyboard():
            ...
            out.append(" ".join([color+key+colorama.Style.RESET_ALL for key, color in items[0:len(items)//3+2]]))
            out.append(" "+" ".join([color+key+colorama.Style.RESET_ALL for key, color in items[len(items)//3+2:2*(len(items)//3)+3]]))
            out.append("  "+" ".join([color+key+colorama.Style.RESET_ALL for key, color in items[2*(len(items)//3)+3:]]))
            return "\n".join(out)
        show = lambda: word*int(self.debug)+str(known)*int(self.debug)+colorama.Fore.BLUE+"="*(self.length*2-1)+colorama.Style.RESET_ALL+"\n"+"\n".join(guesses + [" ".join([WHITE+"-"+colorama.Style.RESET_ALL]*(self.length)) for _ in range(remaining)])+"\n"+colorama.Fore.BLUE+"="*(self.length*2-1)+colorama.Style.RESET_ALL+f"\n{colorama.Fore.MAGENTA}{remaining}{colorama.Fore.CYAN} guesses remaining.{colorama.Style.RESET_ALL}\n{format_keyboard()}\n> "
        for remaining in range(self.length+1, 0, -1):
            while True:
                i+=1
                inp = input(show()).lower()
                if not inp in self.words:
                    utils.clear()
                    print(colorama.Fore.RED+"You're an idiot >:(((. That isn't a valid word."+colorama.Style.RESET_ALL)
                    continue
